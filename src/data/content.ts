...
export const projects: Project[] = [
  {
    ...
    date: "2026",
    oneLine:
      "A curiosity-first learning app, live with **real users**, built on one promise: learn anything in 10 minutes. Each topic unfolds in three swipeable layers, a surprising hook sentence, Instagram-Stories-style cards, then read-aloud, with articles sourced from Wikipedia and hook sentences generated by my own **custom AI model** (Qwen 2.5 7B), grounded in the source. A custom multi-layer prompt engine classifies the topic, curates the richest source sections, and judges every generated quote against an eight-property rubric, while a brain mascot and streak system drive retention and a camera Lens turns photos into articles.",
    problem:
    ...
    decisions:
      "The text layer runs on my own fine-tuned models instead of a paid API, always grounded in Wikipedia and behind a three-layer cache (memory, Supabase, API) so cost stays nearly flat as usage grows; articles, cards, audio, and related topics use free pipelines instead of any model. Those models came out of a training program to own that layer: two open-weights models (Qwen2.5-7B) fine-tuned with LoRA on a distilled, rubric-filtered dataset, one that both classifies the topic and generates the hooks, and a second that judges every quote against the eight-property rubric. I selected the generation winner through a controlled hyperparameter sweep and a blind, judge-scored showdown on held-out topics, then serve them on a scale-to-zero cloud GPU (Modal + vLLM) behind an OpenAI-compatible endpoint.",
    metrics: [
      { value: "2 models", label: "Custom LoRA fine-tunes (Qwen2.5-7B)" },
      { value: "~1,146 rows", label: "Judge-filtered distilled dataset" },
    ],
